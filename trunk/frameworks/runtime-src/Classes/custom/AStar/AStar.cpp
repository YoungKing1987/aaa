...
{
	uint16_t g_value = ((fabs(current_pos.y + current_pos.x - parent_node->pos.y - parent_node->pos.x)) == 2 ? oblique_value_ : step_value_);
...
inline uint16_t AStar::calculHValue(const Vec2 &current_pos, const Vec2 &end_pos)
{
	...
	unsigned int h_value = fabs(end_pos.y - current_pos.y) + fabs(end_pos.x - current_pos.x);
...
bool AStar::canReach(const Vec2 &current_pos, const Vec2 &target_pos, bool corner)
{
	if (target_pos.x >= 0 && target_pos.x < width_ && target_pos.y >= 0 && target_pos.y < height_)
	{
		...
		if (fabs(current_pos.y + current_pos.x - target_pos.y - target_pos.x) == 1)
		{
...
bool AStar::CanWalkableBetween(const Vec2 &sourceTilePos, const Vec2 &destinationTilePos)
{
	...
	// other conditions
//	fabsf(sourceTilePos.x - destinationTilePos.x)>0 ||
